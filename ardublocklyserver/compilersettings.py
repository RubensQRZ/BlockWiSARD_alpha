...
import ardublocklyserver.wisardFoldersToTrain
...
class ServerCompilerSettings(object):
    ...
    #Included by RLQ BlockWiSARD -----------------------------------------
    # Class dictionary to contain the folders with image to train WiSARD, dynamic content
    __wisard_train_folders = {'folder0': 'quadrado'}
    #--------------------------------------------------------------------
    ...
    #
    # IDLE Directory accessors
    #
    def get_IDLE_dir(self):
        return self.__IDLE_dir

    def set_IDLE_dir(self, new_IDLE_dir):
        """Set the IDLE dir, must a valid file or directory."""
        # Mac only check, as apps are packaged directories
        if sys.platform == 'darwin':
            # Arduino version >1.6.0 has changed the binary name, so check both
            bundle = os.path.join(new_IDLE_dir, 'Contents', 'MacOS')
            if os.path.isfile(os.path.join(bundle, 'JavaApplicationStub')):
                new_IDLE_dir = os.path.join(bundle, 'JavaApplicationStub')
                print('IDLE file in OS X located within the app bundle.')
            elif os.path.isfile(os.path.join(bundle, 'Arduino')):
                new_IDLE_dir = os.path.join(bundle, 'Arduino')
                print('IDLE file in OS X located within the app bundle.')
            else:
                print('Could not find Arduino executable in OS X app bundle.')

        # Check directory
        if os.path.isfile(new_IDLE_dir):
            self.__IDLE_dir = new_IDLE_dir
            print('IDLE directory set to:\n\t%s' % self.__IDLE_dir)
            self.save_settings()
        else:
            print('The provided IDLE path is not valid !!!'
                  '\n\t %s' % new_IDLE_dir)
            if self.__IDLE_dir:
                print('Previous IDLE path maintained:\n\t%s' %
                      self.__IDLE_dir)
            else:
                self.set_IDLE_dir_default()
                print('Default IDLE path set:\n\t%s' %
                      self.__IDLE_dir)
                self.save_settings()

    IDLE_dir = property(get_IDLE_dir, set_IDLE_dir)

    def set_IDLE_dir_default(self):
        self.__IDLE_dir = None

    def set_IDLE_dir_from_file(self, new_IDLE_dir):
        """Set the IDLE location, must be full path to an existing file."""
        if os.path.exists(new_IDLE_dir):
            self.__IDLE_dir = new_IDLE_dir
        else:
            print('The provided IDLE path in the settings file is not '
                  'valid:')
            print('\t%s' % new_IDLE_dir)
            self.set_IDLE_dir_default()
            print('Default IDLE path set:\n\t%s' % self.__IDLE_dir)
    ...
    # included by RLQ BlockWiSARD ------------------------------------------
    #
    # BlockWiSARD Images do Trains Folders accessors
    #

    def get_wisard_train_folders(self):
        self.populate_wisard_train_folders()
        return self.__wisard_train_folders


    def populate_wisard_train_folders(self):
        """Populate the folder names  dictionary with the avaiable folders."""
        foler_list = ardublocklyserver.wisardFoldersToTrain.get_folders_list()
        self.__wisard_train_folders = {}
        if foler_list:
            folder_id = 0
            for item in foler_list:
                id_string = 'folder' + str(folder_id)
                self.__wisard_train_folders.update({id_string: item})
                folder_id += 1
    #-----------------------------------------------------------------------
    ...
    def set_sketch_name_default(self):
        ...
        self.__sketch_name = 'BW_PythonCode'
    ...
    def save_settings(self):
        ...
        #Included by RLQ BlockWiSARD ------------
        # Python IDLE section
        settings_parser.add_section('Python_IDLE')
        settings_parser.set(
            'Python_IDLE', 'IDLE_exec_path', '%s' % self.IDLE_dir)
        #----------------------------------------
        # Sketch section
        #Modified by RLQ BlockWiSARD
        settings_parser.add_section('BW_PythonCode')
        settings_parser.set(
            'BW_PythonCode', 'sketch_name', '%s' % self.sketch_name)
        settings_parser.set(
            'BW_PythonCode', 'sketch_directory', '%s' % self.sketch_dir)
        #-------------------------------------------
        # Ardublockly section
    ...
    def read_settings(self):
        ...
        if settings_dict:
            ...
            #Included by RLQ BlockWiSARD ------------------------------------
            self.set_IDLE_dir_from_file(settings_dict['IDLE_exec_path'])
            #----------------------------------------------------------------
        ...
        #Modifyed by RLQ BlockWiSARD ------------------------------
        print('\tIDLE directory: %s' % self.__IDLE_dir)
        #---------------------------------------------------------
    ...
    def get_settings_file_data(self):
        ...
        try:
            ...
            #Included by RLQ BlockWiSARD
            settings_dict['IDLE_exec_path'] =\
                settings_parser.get('Python_IDLE', 'IDLE_exec_path')
            #---------------------------
            ...
            settings_dict['sketch_name'] =\
                settings_parser.get('BW_PythonCode', 'sketch_name')
            settings_dict['sketch_directory'] =\
                settings_parser.get('BW_PythonCode', 'sketch_directory')
            #--------------------------
