...
def launch_server(ip='localhost', port=3000, document_root_=''):
...
def index_redirect():
	...
	#****************************************************
	#---Modified by RLQ BlockWiSARD - changeg Ardublockly -> BlockWiSARD
    redirect('/BlockWiSARD/index.html')
	#--------------------------------------------------

#****************************************************
#---Modified by RLQ BlockWiSARD - changeg Ardublockly -> BlockWiSARD
@app.route('/BlockWiSARD/<file_path:path>')
def static_ardublockly(file_path):
    """Serve the 'BlockWiSARD' folder static files.
    ...
    return static_file(file_path,
                       root=os.path.join(document_root, 'BlockWiSARD'))
#--------------------------------------------------
...
def handler_settings_get_all():
    ...
    response_dict = {
            ...
            'settings': [{
                ...
                'settings_type': 'IDLE',
                'selected': actions.get_IDLE_path()
                #----------------------------
            }, {
                # included by RLQ BLockWiSARD
                'settings_type': 'imagesfolder',
                'options': [{'value': k, 'display_text': v}
                            for k, v in iteritems(actions.getWiSARDTrainFoders())]
                # ----------------------------
            }, {
    ...
    try:
        ...
        if not new_value:
            ...
            #Included by RLQ BlockWiSARD ----------------------
                ...
                set_value = actions.set_IDLE_path(new_value)
...
def handler_code_post():
    ...
    try:
        ...
        try:
            ...
            '''
            success, ide_mode, std_out, err_out, exit_code = \
                actions.python_ide_send_code(sketch_code)
            #-----------------------------
