...
default_sketch_name = 'BW_PythonCode'
...
def create_sketch(sketch_dir, sketch_name=default_sketch_name,
    ...
    if not isinstance(sketch_code, six.string_types) or \
        ...
        # error text modified by RLQ-BlockWiSARD
        print('The Program name or code given is not a valid string !!!')
        return None
    ...
    try:
        ...
        #error text modified by RLQ-BlockWiSARD
        print('Error: %s\nPython Program could not be created !!!' % e)
        return None
    return sketch_path
...
def build_sketch_path(sketch_dir, sketch_name):
    """Create the Python Programa folder required for a valid Sketch.

    If a valid directory is provided, it creates the Python Program folder
    ...
    if os.path.isdir(sketch_dir):
        try:
            ...
            # error text modified by RLQ-BlockWiSARD
            print('Error: %s\nProgram Name could not be processed.' % e)
        else:
            if not os.path.exists(sketch_path):
                os.makedirs(sketch_path)
            #Modified by RLQ BLockWiSARD - changed  ".ino' by '.py'
            sketch_path = os.path.join(sketch_path, sketch_name + '.py')
    else:
        # error text modified by RLQ-BlockWiSARD
        print('The Program directory "%s" does not exists !!!' % sketch_dir)
    return sketch_path
