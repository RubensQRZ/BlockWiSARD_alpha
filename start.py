...
#**************************************************************************
#Modified by RLQ BlockWiSARD  - Modified dir name ardublockly - BlockWiSARD
def find_ardublockly_dir(search_path):
    """Find the BlockWiSARD project directory absolute path.

    Navigates within each node of given path and tries to find the BlockWiSARD
    project root directory. Assumes that the project root will have an folder
    name BlockWiSARD with an index.html file inside.
    ...
    :param search_path: Path starting point to search the BlockWiSARD project
            root folder.
    :return: Path to the BlockWiSARD root folder. If not found returns None.
    """
    path_to_navigate = os.path.normpath(search_path)
    # Navigate through each path node from the bottom up
    while path_to_navigate:
        # Check if file BlockWiSARD/index.html exists within current path
        if os.path.isfile(
                os.path.join(path_to_navigate, 'BlockWiSARD', 'index.html')):
    ...
    if len(sys.argv) == 1:
        ...
        for opt, arg in opts:
            ...
            if opt in ('-s', '--serverroot'):
	         ...
	         #**********************************************************************
			 # Modified by RLQ BlockWiSARD  changed Ardublockly by BlockWiSARD 
                print('Parsed "%s" flag. The BlockWiSARD project root will be '
...
def main():
	...
	#**********************************************************************
	# Modified by RLQ BlockWiSARD  changed the root folder Ardublockly by BlockWiSARD 
	
    # Based on command line options, set the server root to the BlockWiSARD
    ...
    if ardublockly_root_dir is None:
        print('The BlockWiSARD project root folder could not be found within '
              'the %s directory !' % this_file_dir)
        sys.exit(1)
    print('BlockWiSARD root directory:\n\t%s' % ardublockly_root_dir)
    ...
    if find_project_root is True or server_root is None:
        ...
        # Arguments have set a server root, and to not find BlockWiSARD dir
        if not os.path.commonprefix([server_root, ardublockly_root_dir]):
            print('The BlockWiSARD project folder needs to be accessible from '
