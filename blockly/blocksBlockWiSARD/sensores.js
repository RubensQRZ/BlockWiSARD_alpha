...
goog.provide('Blockly.Blocks.Raspberry.Sensores');
...
function findPairBlockBefore(typeSearch,caller) {
  ...
  for (var i = 0; i < listBlocks.length; i++) {
    if (listBlocks[i].type == typeSearch) {
        //array.indexOf(2); --- pra tentar pegar a posicao do pai
        listPos[0] = i+1;
        blocks [0] = 1;
	      break;
...
Blockly.Blocks['le_estado_interruptor'] = {
  init: function() {
    ...
    var colourYellow = new Blockly.FieldColour('#ffffff');
    colourYellow.setColours(['#ff0','#fff']).setColumns(2);
    ...
    var colourBlack = new Blockly.FieldColour('#ffffff');
    colourBlack.setColours(['#000','#fff']).setColumns(2);
    ...
    this.appendDummyInput()
        ...
        .appendField(Blockly.Msg.SWITCH_VERIFY)
        .appendField(Blockly.Msg.SWITCH_SWITCH_BEFORE)
        .appendField(colourGreen, "switchVerde")
        .appendField(colourYellow, "switchAmarelo")
        .appendField(colourRed, "switchVermelho")
        .appendField(colourBlue, "switchAzul")
        .appendField(colourBlue, "switchPreto")
        .appendField(Blockly.Msg.SWITCH_SWITCH_AFTER);
    this.setPreviousStatement(true);
    this.setNextStatement(true);
    this.setTooltip(Blockly.Msg.SWITCH_TOOLTIP);
...
Blockly.Blocks['apresenta_distancia'] = {
      init: function() {
        this.appendDummyInput()
            .appendField(new Blockly.FieldImage("../blockly/blocks/BlockWiSARD/icons/sensor_distancia.png", 15, 15, "*"))
            .appendField(Blockly.Msg.SHOW_DISTANCE_VALUE_VALUE);
        this.setColour(cor_valores);
        this.setOutput(true, 'String');
        this.setTooltip(Blockly.Msg.SHOW_DISTANCE_VALUE_TOOLTIP);
        this.setHelpUrl("");
     },
     ...
     if (GPIOOutputCode == 'C'){
        ...
        this.setDisabled(true);
          ...
          if (this.parentBlock_ == null) {
            this.setWarningText(Blockly.Msg.REQUIRE_WRITE_SPEAK_BLOCK);
            if (!this.isInFlyout && !this.getInheritedDisabled()) {
               this.setDisabled(true);
            }
          } else {
                if (!findPairBlockBefore("le_distancia",this)){
                   this.setWarningText(Blockly.Msg.REQUIRE_DISTANCE_SENSOR_BLOCK);
                   if (!this.isInFlyout && !this.getInheritedDisabled()) {
                       this.setDisabled(true);
                   }
                }else {
...
Blockly.Blocks['apresenta_luminosidade_inicial'] = {
      init: function() {
        this.appendDummyInput()
            .appendField(new Blockly.FieldImage("../blockly/blocks/BlockWiSARD/icons/sensor_luz.png", 15, 15, "*"))
            .appendField(Blockly.Msg.SHOW_INITIAL_LUMINOSITY_VALUE_VALUE);
        this.setColour(cor_valores);
        this.setOutput(true, 'String');
        this.setTooltip(Blockly.Msg.SHOW_INITIAL_LUMINOSITY_TOOLTIP);
        this.setHelpUrl("");
     },
          ...
          if (this.parentBlock_ == null) {
            ...
            if (!this.isInFlyout && !this.getInheritedDisabled()) {
               this.setDisabled(true);
                ...
                if (!findPairBlockBefore("guarda_luminosidade_ambiente",this)){
                   this.setWarningText(Blockly.Msg.REQUIRE_INITIAL_LUMINOSITY_BLOCK);
                   if (!this.isInFlyout && !this.getInheritedDisabled()) {
                       this.setDisabled(true);
                   }
                }else {
...
Blockly.Blocks['apresenta_luminosidade'] = {
      init: function() {
        this.appendDummyInput()
            .appendField(new Blockly.FieldImage("../blockly/blocks/BlockWiSARD/icons/sensor_luz.png", 15, 15, "*"))
            .appendField(Blockly.Msg.SHOW_LUMINOSITY_VALUE_VALUE);
        this.setColour(cor_valores);
        this.setOutput(true, 'String');
        this.setTooltip(Blockly.Msg.SHOW_LUMINOSITY_TOOLTIP);
        this.setHelpUrl("");
     },
          ...
          if (this.parentBlock_ == null) {
            ...
            if (!this.isInFlyout && !this.getInheritedDisabled()) {
               this.setDisabled(true);
                ...
                if (!findPairBlockBefore("le_luminosidade",this)){
                   this.setWarningText(Blockly.Msg.REQUIRE_LUMINOSITY_BLOCK);
                   if (!this.isInFlyout && !this.getInheritedDisabled()) {
                       this.setDisabled(true);
                   }
                }else {
...
Blockly.Blocks['espera_tecla_enter'] = {
  init: function() {
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.appendDummyInput()
 	    .appendField(new Blockly.FieldImage("../blockly/blocks/BlockWiSARD/icons/enter_key.png", 30, 30, "*"))
        .appendField(Blockly.Msg.ENTERKEY);
    this.setColour(cor_sensores);
 this.setTooltip(Blockly.Msg.ENTERKEY_TOOLTIP);
 this.setHelpUrl("");
  }
};
...
Blockly.Blocks['apresenta_texto_teclado'] = {
      init: function() {
        this.appendDummyInput()
            .appendField(new Blockly.FieldImage("../blockly/blocks/BlockWiSARD/icons/teclado.png", 15, 15, "*"))
            .appendField(Blockly.Msg.SHOW_KEYBORDTEXT_TEXT);
        this.setColour(cor_valores);
        this.setOutput(true, 'String');
        this.setTooltip(Blockly.Msg.SHOW_KEYBORDTEXT_TOOLTIP);
        this.setHelpUrl("");
     },
   onchange: function(e) {
    if (!findPairBlockBefore("le_texto_teclado",this)){
           this.setWarningText(Blockly.Msg.REQUIRE_KEYBOARD_INPUT);
           if (!this.isInFlyout && !this.getInheritedDisabled()) {
               this.setDisabled(true);
           }
    }else {
