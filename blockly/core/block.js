...
/** BlockWiSARD
 * Return all labels referenced by this block.
 * @return {!Array.<string>} List of labels names.
 */
Blockly.Block.prototype.getRotulos = function() {
  var labels = [];
  for (var i = 0, input; input = this.inputList[i]; i++) {
    for (var j = 0, field; field = input.fieldRow[j]; j++) {
      if (field instanceof Blockly.FieldRotulo) {
        labels.push(field.getValue());
      }
    }
  }
  return labels;
...
/**BlockWiSARD
 * Notification that a label is renaming.
 * If the name matches one of this block's labels, rename it.
 * @param {string} oldName Previous name of label.
 * @param {string} newName Renamed label.
 */
Blockly.Block.prototype.renameRotulo = function(oldName, newName) {
  for (var i = 0, input; input = this.inputList[i]; i++) {
    for (var j = 0, field; field = input.fieldRow[j]; j++) {
      if (field instanceof Blockly.FieldRotulo &&
          Blockly.Names.equals(oldName, field.getValue())) {
    ...
    if (typeof element == 'string') {
      ...
      do {
        ...
        switch (element['type']) {
          ...
          //BLockWiSARD
          case 'field_Rotulo':
            field = new Blockly.FieldRotulo(element['rotulo']);
            break;
          //----
          case 'field_dropdown':
