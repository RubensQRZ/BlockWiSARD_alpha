...
Ardublockly.init = function() {
  ...
  //Included by RLQ - BlockWiSARD
  Ardublockly.setLanguageRadio(Ardublockly.getUrlLanguage());
  BlockWiSARD.populateGPIOMenu(GPIOOutputCode,Ardublockly.getUrlLanguage());
  //---------
  ...
  Ardublockly.injectBlockly(document.getElementById('content_blocks'),
                            BlockWiSARD.TOOLBOX_XML, '../blockly/');
  //------------
  //Commented by RLQ 0 BlockWiSARD
  ...
  //Included by RLQ BlockWiSARD
  Ardublockly.setDefaultProgramName(Ardublockly.getUrlLanguage());
  BlockWiSARD.setPythonAccordionTitle(GPIOOutputCode,Ardublockly.getUrlLanguage());
  //----------------------------
...
//Included by RLQ - BlockWiSARD
Ardublockly.setDefaultProgramName = function(languageVl) {
  if (languageVl == "en") {
    document.getElementById('sketch_name').value = "program_name";
  }else {
...
Ardublockly.bindActionFunctions = function() {
  ...
  //  ( Included By RLQ DB4K )
   ...
   //  ( Included By RLQ BlockWiSARD )
  Ardublockly.bindClick_('modal_about_open', function() {
    Ardublockly.openSobreModal();
    $('.button-collapse').sideNav('hide');
  });
 ...
 //-------------------------------------------------------------------------
  ...
  //Button Stop - Included by RLQ DB4K
   Ardublockly.bindClick_('button_stop', function() {
     Ardublockly.terminateSubProcess();
  });
 //----------------------------------------------------------------------
  
//Included by RLQ BlockWiSARD --------------------------------------------
  Ardublockly.bindClick_('settings_IDLE_location', function() {
    var el = document.getElementById('settings_IDLE_location');
	el.disabled = true;
    ArdublocklyServer.setIDLELocation(el.value, function(jsonObj) {
          Ardublockly.setIDLELocationHtml(ArdublocklyServer.jsonToHtmlTextInput(jsonObj));
          ArdublocklyServer.requestIDLELocation(function(jsonObj) {
                  Ardublockly.setIDLELocationHtml(
...
//****************************************************************************************
// Include by RLQ DB4K - BlockWiSARD //

/** Sets the Ardublockly server IDE setting to upload and sends the code. */
Ardublockly.ideSendUploadEmptyLoop = function() {
  // Check if this is the currently selected option before edit sever setting
  Ardublockly.setIdeSettings(null, 'upload');
  Ardublockly.shortMessage(Ardublockly.getLocalStr('stopProgram'));
  Ardublockly.resetIdeOutputContent();
  //Sends de code
  ...
  Ardublockly.largeIdeButtonDesable(true);
    ...
    Ardublockly.pythonIdeOutput(dataBack);
  };
...
Ardublockly.loadServerXmlFile = function(xmlFile) {
  ...
  if (Ardublockly.isWorkspaceEmpty()) {
    ...
    Ardublockly.alertMessage(
            //Modified by RlQ - BlockWiSARD
            Ardublockly.getLocalStr('cleanWorkSpaceAndOpenTitle'),
            Ardublockly.getLocalStr('cleanWorkSpaceAndOpenBody'),
			//-------------------------------------------------------------------
...
Ardublockly.loadUserXmlFile = function() {
 ...
 //Modified by RLQ - BlockWiSARD -  Verify if workspace is empty
    var loadXmlfileAccepted = function() {
         selectFile.click();
    };
...
/**
 * Sets the compiler location fGPIO_OPTorm data retrieve from an updated element.
...
Ardublockly.setIDLELocationHtml = function(newEl) {
  if (newEl === null) return Ardublockly.openNotConnectedModal();

  var IDLELocIp = document.getElementById('settings_IDLE_location');
  if (IDLELocIp != null) {
    IDLELocIp.value = newEl.value || IDLELocIp.value ||
        'Please enter the location of the Python IDLE executable';
    IDLELocIp.style.cssText = newEl.style.cssText;
  }
};


// Wisard Train Folder Images

Ardublockly.setImagesToTrainFoldersHtml = function(newEl) {
  if (newEl === null) return Ardublockly.openNotConnectedModal();

  var fodersDropdown = document.getElementById('wisard_train_folder_list');
  if (fodersDropdown !== null) {
    // Restarting the select elements built by materialize
    $('select').material_select('destroy');
    newEl.name = 'ocult_folder_list';
    newEl.id = 'wisard_train_folder_list';
    fodersDropdown.parentNode.replaceChild(newEl, fodersDropdown);
 ...
 */
 ...
 //---------------------------------------------------------------------
  ...
  ArdublocklyServer.sendSketchToServer(
     //Modified by RLQ BlockWiSARD
      Ardublockly.generatePython(), sendCodeReturn);
};
...
Ardublockly.PREV_PYTHON_CODE_ = '';


/**
 * Populate the Python Code and Blocks XML panels with content generated from
 * the blocks.
 */
Ardublockly.renderContent = function() {
  ...
  var pythonCode = Ardublockly.generatePython();
  if (pythonCode !== Ardublockly.PREV_PYTHON_CODE_) {
    var diff = JsDiff.diffWords(Ardublockly.PREV_PYTHON_CODE_, pythonCode);
    ...
    document.getElementById('content_python').innerHTML =
        prettyPrintOne(resultStringArray.join(''), 'cpp', false);
    Ardublockly.PREV_PYTHON_CODE_ = pythonCode;
  }
...
Ardublockly.newSketch = function() {
  ...
  if (blockCount <= 1) {
    ...
    Ardublockly.renderContent();
	document.getElementById('sketch_name').value = Blockly.Msg.DEFAULT_PROGRAM_NAME;
	document.getElementById('sketch_name').size = 16;
  } else if (blockCount > 1) {
	// Modified by RLQ - BlockWiSARD
    Ardublockly.alertMessage(
        Blockly.Msg.DELETE_BLOCKS_ALERT_TITLE ,
        Blockly.Msg.DELETE_BLOCKS_ALERT_TEXT,
        true,
        function() {
          Ardublockly.workspace.clear();
          Ardublockly.renderContent();
...
//**********************************************************************************************
//( Included by RLQ BlockWiSARD ) Saves the Python Code
Ardublockly.saveCodeFileAs = function() {
  var codeFileName = document.getElementById('sketch_name').value;
  var code = Blockly.Python.workspaceToCode(Ardublockly.workspace);
  var blob = new Blob(
      [code],
      {type: 'text/plain;charset=utf-8'});
  saveAs(blob, codeFileName + '.py');
};
//--------------------------------------------------------------------------------------------------
//************************************************************************************************
// ( Included by RLQ BlockWiSARD )- reloads the application actualizing the numbers of the pins used by the blocks (BlockWiSARD_setup.js File)
Ardublockly.reloadPage = function() {
  ...
  if (blockCount == 0) {
    ...
    Ardublockly.alertMessage(
        Blockly.Msg.RELOAD_ALERT_TITLE ,
        Blockly.Msg.RELOAD_ALERT_TEXT,
        true,
        function() {	
		  document.location.reload(true);
		});
...
Ardublockly.terminateSubProcess = function() {
  Ardublockly.setIdeSettings(null, 'upload');
  Ardublockly.shortMessage(Ardublockly.getLocalStr('stopProgram'));
  Ardublockly.resetIdeOutputContent();
  Ardublockly.buttonStopSpinner(true);
  ...
  ArdublocklyServer.terminateSubProcess(sendCodeReturn);
};
