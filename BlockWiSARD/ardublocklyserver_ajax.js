...
ArdublocklyServer.jsonToIdeModal = function(jsonObj) {
  ...
  if (jsonObj.errors) {
    ...
    if (jsonObj.ide_mode == 'upload') {
      elTitle.innerHTML = Ardublockly.getLocalStr('pythonOpUploadedTitle');
    } else if (jsonObj.ide_mode == 'verify') {
      elTitle.innerHTML = Ardublockly.getLocalStr('pythonOpVerifiedTitle');
    } else if (jsonObj.ide_mode == 'open') {
      elTitle.innerHTML = Ardublockly.getLocalStr('pythonOpOpenedTitle');
      // This is a corner case where we also add to the stand out
      elStdOp.innerHTML += Ardublockly.getLocalStr('pythonOpOpenedBody');
    } else {
      elTitle.innerHTML = Ardublockly.getLocalStr('pythonOpErrorTitle');
    }
  } else {
    console.error(jsonObj);
...
ArdublocklyServer.requestIDLELocation = function(callback) {
  ArdublocklyServer.getJson('/settings/IDLE', callback);
};

ArdublocklyServer.setIDLELocation = function(new_path, callback) {
    ArdublocklyServer.putJson(
      '/settings/IDLE', {"new_value": new_path}, callback);
};

ArdublocklyServer.requesWiSARDImagestoTrainFolders = function(callback) {
  ArdublocklyServer.getJson('/settings/imagesfolder', callback);
};
...
//BlockWiSARD
ArdublocklyServer.terminateSubProcess = function(callback) {
  ArdublocklyServer.sendRequest(
      '/terminate', 'POST', 'application/json', {"action": 'terminate_process'}, callback);
};