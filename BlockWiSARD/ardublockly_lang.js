...
//Changed by RLQ BlockWiSARD -  excluded some language options
...
/**
 * Selected language, default Portuguese.
 * @type {string}
 */
Ardublockly.LANG = 'pt';
...
Ardublockly.initLanguage = function() {
  ...
  Ardublockly.LANG =  Ardublockly.getUrlLanguage() || Ardublockly.LANG ||
...
Ardublockly.injectLanguageJsSources = function(langKey) {
  ...
  var appLangJdPath = 'msg/BlockWiSARD/' + langKey + '.js';
  try {
  ...
  blocklyLangJsLoad.src = '../blockly/msg/BlockWiSARD/' + langKey + '.js';
...
/** Saves the blocks and reloads with a different language. */
Ardublockly.changeLanguageWithId = function(langId) {
  // Store the blocks for the duration of the reload only
  Ardublockly.saveSessionStorageBlocks();
  var newLang = langId;
  var search = window.location.search;
...
Ardublockly.setLanguageRadio = function (langId){
   if (langId == "en"){
      document.getElementById("radioInglish").checked=true;
      document.getElementById("radioPortuguese").checked=false;
    }else{
      document.getElementById("radioInglish").checked=false;
      document.getElementById("radioPortuguese").checked=true;
    }
}
